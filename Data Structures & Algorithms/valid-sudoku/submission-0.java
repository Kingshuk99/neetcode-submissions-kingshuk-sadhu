    ...
    public boolean isValidSudoku(char[][] board) {
        ...
        for(int r=0; r<9; r++) {
            for(int c=0; c<9; c++) {
                ...
                rows.computeIfAbsent(r, key -> new HashSet<>()).add(cell);
                cols.computeIfAbsent(c, key -> new HashSet<>()).add(cell);
                squares.computeIfAbsent((r/3)*3+c/3, key -> new HashSet<>()).add(cell);
            }
        }
